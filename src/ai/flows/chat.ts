...
const getDonationStatusTool = ai.defineTool(
    {
        ...
        description: 'Get the status of the most recent toy donations for the current user.',
...
const schedulePickupTool = ai.defineTool(
    {
        name: 'schedulePickup',
        description: 'Schedules a toy pickup for a user. Collect all required information first.',
        inputSchema: z.object({
            pickupDate: z.string().describe("The desired date for the pickup in YYYY-MM-DD format."),
            timeSlot: z.string().describe("The desired time slot, e.g., '9am - 12pm'."),
            pickupType: z.enum(['my-address', 'drop-off', 'partner']).describe("The type of pickup location."),
            address: z.string().optional().describe("The user's full address, required if pickupType is 'my-address'."),
            locationId: z.number().optional().describe("The ID of the drop-off location, required if pickupType is 'drop-off'."),
            partnerId: z.number().optional().describe("The ID of the partner organization, required if pickupType is 'partner'."),
            notes: z.string().optional().describe("Any notes or special instructions from the user."),
        }),
    ...
    async (input) => {
        const user = await getCurrentUser();
        if (!user) {
            return { success: false, message: "User must be logged in to schedule a pickup." };
        }
        ...
        try {
            ...
            return { success: true, message: `Pickup successfully scheduled on ${input.pickupDate} between ${input.timeSlot}.` };
        } catch (error) {
            console.error("Failed to schedule pickup via chat:", error);
            return { success: false, message: "Sorry, I encountered an error while scheduling the pickup. Please try again." };
        }
    }
);
