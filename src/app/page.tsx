...
const workflowSteps = [
  {
    ...
    title: "Collection",
    description:
      "Toys are collected from generous donors through scheduled pickups and drop-off locations.",
  },
  {
    icon: <Sparkles className="h-10 w-10 text-primary" />,
    title: "Sanitization",
    description:
      "Every toy is thoroughly cleaned and sanitized to ensure it is safe and hygienic for the next child.",
  },
  {
    icon: <Warehouse className="h-10 w-10 text-primary" />,
    title: "Warehouse",
    description:
      "Cleaned toys are sorted and cataloged in our warehouse, ready for the next step.",
  },
  {
    icon: <Wrench className="h-10 w-10 text-primary" />,
    title: "Refurbishing",
    description:
      "Our team lovingly repairs and refurbishes toys that need a little extra care.",
  },
  {
    icon: <Gift className="h-10 w-10 text-primary" />,
    title: "Ready for Reuse",
    description:
      "Toys are redistributed to our partner organizations to bring joy to new families.",
  },
];
      ...
      <div className="flex flex-col gap-8 animate-fade-in">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          <Card className="transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
            ...
            <CardContent>
              <div className="text-3xl font-bold">
                {stats.toysRedistributed.toLocaleString()}
              </div>
          ...
          <Card className="transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
            ...
            <CardContent>
              <div className="text-3xl font-bold">
                ~{stats.smilesCreated.toLocaleString()}
              </div>
              <p className="text-xs text-muted-foreground">
                Joy delivered to children
              </p>
            </CardContent>
          </Card>
          <Card className="md:col-span-2 lg:col-span-1 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
            ...
            <CardContent>
              <div className="text-3xl font-bold">
                {stats.environmentalImpact.split(" ")[0]} tons
              </div>
              <p className="text-xs text-muted-foreground">
        ...
        <section className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              The journey of a toy from one child to the next.
            </p>
          </div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5">
            {workflowSteps.map((step, index) => (
              <Card
                key={index}
                className="text-center flex flex-col items-center p-6 transform transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl"
              >
