...
const formActionState = z.object({
  message: z.string(),
  error: z
    .union([z.string(), z.record(z.array(z.string())), z.undefined()])
    .optional(),
});
...
export async function createEcommerceIntegration(
  prevState: z.infer<typeof formActionState>,
  formData: FormData
) {
  const validatedFields = ecommerceIntegrationSchema.safeParse(
...
export async function updateEcommerceIntegration(
  id: number,
  prevState: z.infer<typeof formActionState>,
  formData: FormData
) {
