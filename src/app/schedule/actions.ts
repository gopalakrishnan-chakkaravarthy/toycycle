...
const mockLocations = [
  {
    ...
    address: "4500 Northwood Ave",
    hours: "",
  },
  {
    id: 2,
    name: "Southside Public Library",
    address: "876 Library Ln",
    hours: "",
  },
];
...
export async function getScheduleData() {
  ...
  try {
    ...
    return {
      toyConditions:
        toyConditions.length > 0 ? toyConditions : mockToyConditions,
      accessoryTypes:
        accessoryTypes.length > 0 ? accessoryTypes : mockAccessoryTypes,
...
export async function getScheduledDays() {
  ...
  try {
    const result = await db
      .selectDistinct({ date: pickups.pickupDate })
      .from(pickups);
...
export async function getPickupsForDate(
  user: User | null,
  date: Date | undefined
...
export async function schedulePickup(
  validatedData: z.infer<typeof schedulePickupSchema>
  ...
  if (process.env.POSTGRES_URL) {
    try {
      ...
      const cost: number | null =
        collectionCost && !isNaN(collectionCost) ? collectionCost : null;
      ...
      return {
        success: false,
        message:
          "Could not save the appointment to the database. Please try again later.",
      };
    }
  }
  ...
  if (process.env.SENDGRID_API_KEY && process.env.SENDGRID_FROM_EMAIL) {
    ...
    if (pickupType === "my-address") {
      ...
      const location = await db.query.locations.findFirst({
        where: (l, { eq }) => eq(l.id, parseInt(locationId)),
      });
      ...
      const partner = await db.query.partners.findFirst({
        where: (p, { eq }) => eq(p.id, parseInt(partnerId)),
      });
    ...
    try {
      ...
      return {
        success: true,
        message: "Pickup scheduled and email sent successfully.",
      };
    } catch (error) {
      console.error("SendGrid Error:", error);
      return {
        success: true,
        message: "Pickup scheduled, but failed to send confirmation email.",
      };
    }
  }
